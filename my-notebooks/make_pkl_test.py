...
test_meta = pd.read_csv(os.path.join(root,'data/mimic-cxr/splits/test.csv'))
path = os.path.join(root, 'data/mimic-cxr/pkls/')

images = []
for i in range(len(test_meta)):
    
    img = cv2.imread(test_meta.iloc[i]['path'],cv2.IMREAD_GRAYSCALE) #so it only has one channel
